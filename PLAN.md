...
- **프레임 캡을 목표 FPS에 걸지 않는다.** 캡을 목표에 두면 측정값이 목표를 넘을 수 없고, 상향 조건(목표 대비 여유)이 영원히 성립하지 않는다. 화질이 한 방향으로만 내려가 고착된다.
- **밉맵은 적응 대상에서 뺀다.** 변경이 전체 리소스 리로드를 요구한다. 끊김을 막으려다 끊김을 만든다.
- **판정 지표는 평균이 아니라 5% low.** "목표 FPS 보장"을 *프레임 20개 중 19개가 제때 도착한다*로 정의한다.
...
3. 품질 유니폼을 매 프레임 밀어 넣을지(`WorldRenderEvents.START` + `GlUniform`), 아니면 세션 시작 시 팩 하나를 고르고 끝낼지
