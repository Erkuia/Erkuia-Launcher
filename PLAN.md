...
| L3-2 | 설정 저장/로드 | `config.json` — 모드 상태, 선택 계정. 원자적 쓰기 |
...
| ID | 작업 | 내용 |
| --- | --- | --- |
| L6-1 | 모드 모델 | 필수 런처 소유 모드 / 로컬 추가 모드 2분류 |
...
| L8-2 | 모드 설정 파일 생성 | 서버 주소를 런처 소유 모드가 읽을 형식으로 기록 |
...
**만들었다가 걷어냈다.** 기록으로 남긴다 — 다시 시도할 사람이 같은 길을 처음부터 걷지 않도록.

목표는 목표 FPS에 맞춰 화질을 자동으로 깎는 것이었다. 처음에는 `GameOptions`(렌더 거리·파티클·구름 등)를 조정했지만, 그건 플레이어가 직접 고른 설정을 말없이 덮어쓰는 일이라 방향을 틀었다. 대신 런처가 소유한 지렛대 — 렌더 해상도 스케일링 — 를 만들기로 했다.

네 번 시도해 네 번 다 실패했고, 마지막에 계측을 넣고서야 이유를 알았다.
...
| 0.50 | 427x240 | 28 |
| 0.35 | 299x168 | 26 |
| 0.35 (이중 스케일 버그) | **105x59** | 12 |

마지막 줄이 결론이다. 월드를 **6천 픽셀**로 그려도 12 FPS 였다. 픽셀이 조금이라도 병목이었다면 그 조건에서 수백이 나왔어야 한다. 이 클라이언트는 청크·엔티티·모드의 CPU 작업에 묶여 있고, 프래그먼트 처리는 병목이 아니다.

### 남은 교훈

- **프레임 그래프는 메인 프레임버퍼 크기를 따라간다.** 1.21.2+ 의 `WorldRenderer` 는 반투명·파티클·날씨·구름·엔티티 아웃라인 타깃을 메인 크기에서 파생시킨다. 메인만 바꿔치기하면 나머지는 알아서 따라온다 — 여기에 `SimpleFramebufferFactory` 까지 손대면 배율이 두 번 곱해진다.
- **그럼에도 월드는 축소 버퍼에 그려지지 않았다.** 뷰포트는 축소 크기로 잡혔는데 화면은 원해상도로 선명했다. `renderWorld` 진입점이 실제 렌더 대상이 정해지는 지점이 아니라는 뜻이고, 이 부분은 끝내 규명하지 못했다.
- **Sodium 은 참고 대상이 아니다.** 해상도 스케일링 기능 자체가 없고(청크 렌더링 엔진 교체 모드다), 라이선스도 Polyform Shield 라 코드 참조가 부적절하다.

### 다시 손대려면

먼저 **1080p 이상에서 픽셀이 병목인지부터 재라.** 위 표가 아니라고 말하고 있으므로, 그 전제가 뒤집히지 않으면 구현은 의미가 없다.
